...
exports.createTeam = async (req, res) => {
  try {
    ...
    if (!name || !eventId) {
      return res
        .status(400)
        .json({ error: "Team name and event are required" });
    }
...
exports.getTeamByInviteCode = async (req, res) => {
  try {
    const inviteCode = req.params.inviteCode.toLowerCase();

    const team = await Team.findOne({ inviteCode })
      .populate(
        "event",
        "name maxTeamSize minTeamSize registrationDeadline isActive",
      )
...
exports.joinTeam = async (req, res) => {
  try {
    ...
    const team = await Team.findOne({ inviteCode }).populate(
      "event",
      "name maxTeamSize registrationDeadline isActive",
    );
...
exports.getMyTeams = async (req, res) => {
  try {
    ...
    const teams = await Team.find({ "members.user": userId })
      .populate("event", "name startDate endDate status isActive maxTeamSize")
      .populate("members.user", "name email image");
...
exports.removeMember = async (req, res) => {
  try {
    ...
    if (!requester) {
      return res
        .status(403)
        .json({ error: "Only team lead can remove members" });
    }
    ...
    if (memberId === userId) {
      return res
        .status(400)
        .json({ error: "Use leave team to remove yourself" });
    }
